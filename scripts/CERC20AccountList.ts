...
import { HDNodeWallet, Signer, Wallet, keccak256 } from "ethers";
...
async function main() {
  ...
  const masterNode = HDNodeWallet.fromPhrase(mnemonic, "m/44'/60'/0'");
  const wallets: Wallet[] = [];
  for (let i = 0; i < 2; i++) {
    // 相对派生路径 "0/i" 最终组合出 "m/44'/60'/0'/0/i"
    const derivedWallet = masterNode.derivePath(`0/${i}`);
    // 使用 provider 构造钱包对象
    const wallet = new Wallet(derivedWallet.privateKey, provider);
    wallets.push(wallet);
  }
  const ethers_wallet = wallets[0];
  const user1 = wallets[1];
  console.log("Wallet 0 address:", wallets[0].address);
  console.log("Wallet 1 address:", wallets[1].address);
