...
async function main() {
  ...
  const localSightEVM: HttpNetworkUserConfig = hre.userConfig.networks!.localSightEVM! as HttpNetworkUserConfig;

  const provider = new ethers.JsonRpcProvider(localSightEVM.url);
  ...
  const providerKey = new ethers.JsonRpcProvider(localSightFHEVM.url);
  ...
  const network1 = await providerKey.getNetwork();
  chainId = +network1.chainId.toString(); // Need to be a number
  try {
    // Get blockchain public key
    const ret = await providerKey.call({
      to: FHE_LIB_ADDRESS,
